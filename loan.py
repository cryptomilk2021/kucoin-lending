...
class Loan:
    ...
    def start_log(self):
        write_to_log()
        write_to_log(
            f'----------Created {self.coin} instance, deleting orders older than {self.delete_orders_older_than_hours} hours----------')
    ...
    def get_available_balance(self):
        ...
        write_to_log(
            f'{round(float(self.account_balance["available"]), 2)} {self.coin} available for lending, lend precision {self.precision}')
    ...
    def lend_coin(self):
        ...
        def split_amount(amt, precision):
            ...
            write_to_log('cannot lend, not enough assets')
            return None
                ...
                for i in range(split_amounts[0]):
                    ...
                    if lend_amount > 0:
                        ...
                        if order_nbr.json()['code'] == '400100':
                            write_to_log(
                                f'Failed Lending {lend_amount} {self.coin}, coin precision was unknown, recorded in min_lend_amount.json for future use')
                        ...
                        if 'data' in order_nbr.json():
                            ...
                            write_to_log(
                                f'Lending {lend_amount} {self.coin}, @ {lend_interest_rate_daily}% daily ({round(apr, 2)}%APR), 7 days, order {a}')
    ...
    def cancel_old_orders(self):
        ...
        for index in range(len(df)):
            ...
            if dict_order['dailyIntRate'] == lowest_rate:
                write_to_log(
                    f'not deleting {dict_order["size"]} {self.coin}, {dict_order["orderId"]}, currently at lowest available rate {lowest_rate}% daily')
                continue
